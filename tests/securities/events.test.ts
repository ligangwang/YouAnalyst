...
test("transaction summaries retain source codes, date and quality without inventing purchase totals", async () => {
 const row = {accessionNumber:input.accessionNumber,reportingOwnerName:"Jane Example",transactionCode:"A",transactionDate:"2026-09-03",securityTitle:"Common stock",shares:1200,valueUsd:100000,valueQuality:"needs_review",privateNote:"never expose",updatedAt:now};
 ...
 const db = {collection:(name:string)=>{
   if(name === "sec_insider_filings") return {doc:(id:string)=>{assert.equal(id,input.accessionNumber);return {get:async()=>({data:()=>({status:"PARSED",parsedAt:now})})};}};
   assert.equal(name,"insider_transactions");
   const filters: unknown[][] = [];
   const query = {where:(...args:unknown[])=>{filters.push(args);return query;},limit:(limit:number)=>{assert.equal(limit,5);assert.deepEqual(filters,[["accessionNumber","==",input.accessionNumber],["updatedAt","==",now]]);return {get:async()=>{reads++;return {docs:[{data:()=>row},{data:()=>({...row,updatedAt:"2025-01-01T00:00:00.000Z",reportingOwnerName:"Stale row"})}]};}};}};
   return query;
 }} as unknown as Firestore;
 ...
 assert.equal(filingEvent({...input,transactionParsedAt:now},now).transactionParsedAt,now);
 assert.equal(publicEventFromDocument(event.id,{...event,transactionParsedAt:now})?.transactionParsedAt,now);
});
