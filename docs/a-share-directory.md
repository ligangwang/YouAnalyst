...
3. `/admin/industry-research` retains US industry connection research and uses imported CNI industries for A-share connections. Relationships require searched evidence and admin review. Industry membership never creates a relationship. A-share relationships are saved separately in `market_company_relationships`; the public US graph is not mixed with these IDs.
...
Published profiles stay in `market_companies`. Imported identities are never exposed as researched profiles. Admin approval creates missing profiles while preserving editorial content. The five-company seed remains an idempotent deployment migration. CNI classifications are never represented as GICS.
