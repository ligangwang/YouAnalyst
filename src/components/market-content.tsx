...
export function MarketContent({ children }: { children: ReactNode }) {
  const { market } = useMarket(); const { text } = useLocale(); const path = unlocalizedPath(usePathname());
  const discovery = path === "/predictions" || path === "/predictions/new" || path === "/institutions" || path === "/leaderboard" || path.startsWith("/daily");
  ...
  if (market === "CN_A" && discovery) return <main className="mx-auto max-w-3xl px-4 py-16">
    ...
    <Link href="/map" className="mt-6 inline-block rounded-full border border-white/20 px-5 py-3 text-sm">{text("Explore the global AI supply chain", "探索全球 AI 产业链")}</Link>
  </main>;
  return <>{market !== "US" && path !== "/map" && path !== "/" && <p role="status" className="mx-auto max-w-6xl px-4 pt-4 text-xs text-slate-400">{text("Available events, prices and calls currently cover US securities. A-share research is available in Explore. Existing records keep their own market.", "当前动态、行情与观点数据覆盖美股；A 股研究可在“探索”中查看。已有记录保留其所属市场。")}</p>}{children}</>;
}
