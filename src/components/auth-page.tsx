...
export function AuthPage({ requestedNext, initialCreate = false }: { requestedNext?: string; initialCreate?: boolean }) {
  ...
  useEffect(() => {
    ...
    void fetch("/api/knowledge-graph", {signal:controller.signal}).then(r => r.ok ? r.json() : null).then((graph: KnowledgeGraph | null) => {
      const node = graph?.nodes?.find(n => n.id === followId);
      if (node && !controller.signal.aborted) setFollowCompany({id:followId,name:companyName(node,chinese ? "zh-CN" : "en")});
    }).catch(() => {});
    return () => controller.abort();
  }, [followId, chinese]);
  const followName = followCompany && followCompany.id === followId ? followCompany.name : followId?.startsWith("ORG:") ? (chinese ? "这家公司" : "this company") : followId?.split(":").at(-1);
