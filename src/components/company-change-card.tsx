...
export function CompanyChangeCard({ item, graph }: { item: CompanyUpdate; graph: KnowledgeGraph }) {
  ...
  return <article className="rounded-xl border border-white/10 p-5">
    ...
    <h3 className="mt-2 font-semibold"><Link href={item.href} onClick={() => { trackEvent("company_event_open", {entry_point:"following"}); if (event) trackEvent("company_event_map", {entry_point:"following"}); }} className="hover:text-cyan-200">{title}</Link></h3>
    <p className="mt-2 text-sm leading-6 text-slate-300">{event && chinese ? event.summaryZh : item.description}</p>
    <ul className="mt-3 space-y-1 border-l-2 border-cyan-800 pl-3 text-xs text-slate-300" aria-label={text("Why this appears", "为何关联到你")}>
    ...
    {item.reasons?.some(r => r.edgeId) && <p className="mt-2 text-xs text-slate-400">{text("One-hop context only; this does not establish an order or financial impact on your followed company.", "仅提示一跳产业链背景，不代表你关注的公司获得订单或受到财务影响。")}</p>}
