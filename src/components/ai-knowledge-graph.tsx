...
export function AiKnowledgeGraph({ initialMarket = "ALL", initialCompany = "", initialView = "3d" }: { initialView?: "2d" | "3d"; initialMarket?: "ALL" | "NONE" | Market; initialCompany?: string }) {
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.controls}>
      ...
      <div className={styles.markets} aria-label={text("Graph view", "图谱视图")}>{(["3d", "2d"] as const).map(mode => <button key={mode} aria-pressed={view === mode} onClick={() => { setView(mode); setZoom(100); setRotation(0); }}>{mode.toUpperCase()}</button>)}</div>
      ...
      <a className={styles.filingLink} href="/map?view=filings">{text("Filing explorer", "财报关系探索")}</a><div className={styles.zoom}><button aria-label={text("Zoom out", "缩小")} disabled={zoom <= 60} onClick={() => setZoom(z => Math.max(60, z - 20))}>−</button><output>{view === "2d" ? `${zoom}%` : "3D"}</output><button aria-label={text("Zoom in", "放大")} disabled={zoom >= 320} onClick={() => setZoom(z => Math.min(320, z + 20))}>+</button><button aria-label={text("Rotate left", "向左旋转")} onClick={() => rotate(-15)}>↶</button>{view === "2d" && <output aria-label={text("Rotation", "旋转角度")}>{rotation}°</output>}<button aria-label={text("Rotate right", "向右旋转")} onClick={() => rotate(15)}>↷</button><button onClick={() => { setZoom(100); setRotation(0); if (view === "3d") setSelected(""); setReset(n => n + 1); }}>{text("Fit", "全图")}</button></div>
    </div>
    ...
    {status === "loading" ? <p className={styles.empty} role="status">{text("Loading the knowledge graph…", "正在加载知识图谱…")}</p> : status === "error" ? <div className={styles.empty} role="alert">{text("The graph could not be loaded.", "暂时无法加载图谱。")} <button onClick={() => { setStatus("loading"); setRetry(n => n + 1); }}>{text("Try again", "重试")}</button></div> : !visible.nodes.length ? <p className={styles.empty}>{!markets.length ? text("Turn on a market to explore its companies.", "开启一个市场以查看公司。") : text("No matching companies.", "没有匹配的公司。")}</p> : <div className={`${styles.workspace} ${company ? styles.withDetail : ""}`}>
      {view === "3d" ? <Suspense fallback={<p className={styles.empty} role="status">{text("Loading 3D…", "正在加载 3D…")}</p>}><CompanyGraph3D graph={visible} selected={company?.id ?? ""} onSelect={setSelected} zoom={zoom} rotation={rotation} reset={reset} onFallback={() => setView("2d")}/></Suspense> : <CompanyConstellation key={`${markets.join(",")}:${query}:${reset}`} graph={visible} selected={company?.id ?? ""} onSelect={setSelected} zoom={zoom} rotation={rotation} onRotate={rotate} />}
