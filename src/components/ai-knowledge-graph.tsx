...
export function AiKnowledgeGraph({ initialMarket = "ALL", initialCompany = "", initialQuery = "" }: { initialMarket?: "ALL" | "NONE" | Market; initialCompany?: string; initialQuery?: string }) {
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.controls}>
      ...
      <input aria-label={text("Search companies", "搜索公司")} placeholder={text("Search company or ticker…", "搜索公司或股票代码…")} value={query} onChange={e => { const value = e.target.value; setQuery(value); const url = new URL(window.location.href); if (value) url.searchParams.set("q", value); else url.searchParams.delete("q"); window.history.replaceState(null, "", url); }}/>
    </div>
