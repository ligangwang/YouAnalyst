...
export function AiKnowledgeGraph({ initialMarket = "ALL", initialCompany = "" }: { initialMarket?: "ALL" | "NONE" | Market; initialCompany?: string }) {
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.controls}>
      ...
      <a className={styles.filingLink} href="/map?view=filings">{text("Filing explorer", "财报关系探索")}</a><div className={styles.zoom}><button aria-label={text("Zoom out", "缩小")} disabled={zoom <= 60} onClick={() => setZoom(z => Math.max(60, z - 20))}>−</button><output>{zoom}%</output><button aria-label={text("Zoom in", "放大")} disabled={zoom >= 320} onClick={() => setZoom(z => Math.min(320, z + 20))}>+</button><button onClick={() => { setZoom(100); setReset(n => n + 1); }}>{text("Fit", "全图")}</button></div>
    </div>
