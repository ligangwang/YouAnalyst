...
export function ChinaSupplyChain({ embedded = false, initialQuery = "" }: { embedded?: boolean; initialQuery?: string }) {
  ...
  return <Container id={embedded ? "a-share-companies" : undefined} aria-labelledby={embedded ? "a-share-heading" : undefined} className="mx-auto max-w-6xl px-4 py-10 sm:py-16">
    ...
    {companies.length > visibleCount && <button className="mt-6 rounded-full border border-white/20 px-5 py-3" onClick={() => setVisibleCount(n => n + 100)}>{text("Show more companies", "显示更多公司")}</button>}
