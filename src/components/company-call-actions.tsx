      ...
      {items?.map(call => {
        ...
        return <article key={call.id} aria-label={`${call.watchlistName}: ${label}`} className={`rounded-xl border p-4 ${call.direction === "UP" ? "border-emerald-400/35 bg-emerald-400/5" : "border-rose-400/35 bg-rose-400/5"}`}>
          ...
          {call.status === "CLOSING" ? <p className="mt-3 text-amber-200"><UiText text={"Closing — awaiting end-of-day settlement."} /></p>
            ...
            : editing !== call.id ? <button type="button" disabled={!!pending || call.entryPrice === null || !call.entryDate} onClick={() => { setEditing(call.id); setReason(""); }} className="mt-3 min-h-11 rounded-lg border border-white/25 px-4 font-semibold text-white disabled:opacity-50"><UiText text={"Close "} />{<UiText text={label} />}</button>
              : <form className="mt-3 grid gap-2" onSubmit={event => { event.preventDefault(); void act(call, "close"); }}>
                <label htmlFor={`close-${call.id}`} className="text-slate-200"><UiText text={"Why are you closing this call?"} /></label>
