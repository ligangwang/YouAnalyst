...
const handlers = createMapFollowHandlers({
  ...
  async read(uid) { return (await getAdminFirestore().collection("industry_map_preferences").doc(uid).get()).data()?.followedCompanyIds; },
  async exists(id) {
  ...
  async update(uid, id, follow) {
    const db = getAdminFirestore(); const ref = db.collection("industry_map_preferences").doc(uid);
