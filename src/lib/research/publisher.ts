...
// Firestore may reorder map keys. Hash values independently of object insertion order.
export function hash(v: unknown): string {
  ...
  return createHash("sha256").update(JSON.stringify(sort(JSON.parse(JSON.stringify(v))))).digest("hex");
}
