...
export function isLocalizedPage(path: string): boolean {
  return path === "/" || /^\/(research|posts|compare|my|map|companies|company|ticker|feed|daily|watchlists|how-it-works|feedback|analysts|predictions|leaderboard)(\/|$)/.test(path);
}
