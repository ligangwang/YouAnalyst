...
import type { DailyInstitutionalMoveShareKind } from "@/lib/daily-scores/public-share";
...
export async function DailyInstitutionalMoveShareView({
  date,
  kind,
  snapshot,
  ticker,
  ...
  date: string;
  kind: DailyInstitutionalMoveShareKind;
  snapshot: DailyInstitutionalMove | null;
  ticker: string;
}) {
  const normalizedTicker = ticker.trim().toUpperCase();
